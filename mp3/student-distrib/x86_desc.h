...
/* PDE ENTRY FOR A 4KB ENTRY, WHICH LEAD TO A PT */
typedef union PDE_4KB_T {
    ...
    struct {
        ...
        uint32_t availablr_for_user             :3;
        uint32_t PB_addr                        :20;
    } __attribute__ ((packed));
} PDE_4KB_T;


/* PDE ENRTY FOR A 4KB ENTRY, LEAD TO MEMORY */
...
// this further defines the format the page_d and page_table
extern PDE_T page_directory[PDE_NUM] __attribute__((aligned (FOURKB)));
extern PTE_T page_table[PTE_NUM] __attribute__((aligned (FOURKB)));
extern PTE_T page_table_usermap[PTE_NUM] __attribute__((aligned (FOURKB)));
